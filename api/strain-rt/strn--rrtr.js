...
strn.post("/", (req, res) => {
  ...
  if (!strain_name || !strain_type) {
    ...
    res
      .status(400)
      .json({ message: "please add effects and flavors new strain" });
  } else {
    let strain_id;
    helper
      .add(strain)
      ...
      .then(rem => {
        if (rem > 0) {
          res
            .status(200)
            .json({
              message: `Strain with id of ${id} has been removed`,
              id: id
            });
        } else {
          res
            .status(404)
            .json({ errorMessage: `Strain with ${id} could not be found` });
        }
      })
      .catch(err => {
        res.status(500).json({ error: "failed to delete strain from db" });
      });
  });
});
